...
public class EffectHelper {
    ...
    public static void playSporeContagion(Entity source, List<LivingEntity> targets, double radius) {
        ...
        for (int i = 0; i < ringPoints; i++) {
            ...
            if (RANDOM.nextFloat() < 0.5f) {
                ...
                level.sendParticles(new DustParticleOptions(dustColor(deepColor), 2.0f),
                        x + ox, baseY + 0.1, z + oz, 1, 0, 0, 0, 0);
            }
            if (RANDOM.nextFloat() < 0.3f) {
                level.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0xFFFFFFFF),
        ...
        for (LivingEntity target : targets) {
            ...
            for (int j = 0; j <= linePoints; j++) {
                ...
                if (j % 2 == 0) {
                    level.sendParticles(new DustParticleOptions(dustColor(SMOG_COLOR), 0.8f), lx, ly, lz, 1, 0, 0, 0, 0);
                } else {
                    if (RANDOM.nextFloat() < 0.1f) {
                        level.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0xFFFFFFFF), lx, ly, lz, 0, 0.2, 0.9, 0.2, 1.0);
                    }
                }
            }
        }
    ...
    public static void playParalysisAmbient(Entity entity, int amplifier) {
        ...
        if (RANDOM.nextFloat() < ElementalVisualConfig.paralysisDustChance) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                level.sendParticles(new DustParticleOptions(dustColor(PARALYSIS_YELLOW), size),
    ...
    public static void playToxicBlast(Level level, Vec3 pos, double radius) {
        ...
        serverLevel.sendParticles(ColorParticleOption.create(ParticleTypes.FLASH, 0xFFFFFFFF), pos.x, pos.y, pos.z, 1, 0, 0, 0, 0);
    ...
    private static void spawnSmokeBatch(ServerLevel serverLevel, double x, double y, double z,
        ...
        for (int i = 0; i < batchCount; i++) {
            double dx = (serverLevel.getRandom().nextDouble() - 0.5) * radius * 1.5;
            double dy = serverLevel.getRandom().nextDouble() * 1.5;
            double dz = (serverLevel.getRandom().nextDouble() - 0.5) * radius * 1.5;
            double vx = dx * 0.015;
            double vy = 0.01 + serverLevel.getRandom().nextDouble() * 0.02;
            double vz = dz * 0.015;
    ...
    private static void spawnNatureCore(ServerLevel level, Vec3 pos) {
        for (int angle = 0; angle < 360; angle += 30) {
            ...
            if (RANDOM.nextFloat() < 0.5f) {
                level.sendParticles(new DustParticleOptions(dustColor(SMOG_COLOR), 2.0f), x, pos.y + 0.08, z, 1, 0, 0, 0, 0);
            }
        }
        if (RANDOM.nextFloat() < 0.4f) {
            level.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0xFFFFFFFF), pos.x, pos.y + 0.5, pos.z, 0, 0.2, 0.9, 0.2, 1.0);
        }
        if (RANDOM.nextFloat() < 0.3f) {
    ...
    private static void spawnNatureRing(ServerLevel level, Vec3 pos, double r) {
        for (int angle = 0; angle < 360; angle += 10) {
            ...
            if (RANDOM.nextFloat() < 0.65f) {
                level.sendParticles(new DustParticleOptions(dustColor(SMOG_COLOR), 1.5f), x, pos.y + 0.08, z, 1, 0, 0, 0, 0);
            }
            if (RANDOM.nextFloat() < 0.12f) {
                level.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0xFFFFFFFF), x, pos.y + 0.2, z, 0, 0.2, 0.9, 0.2, 1.0);
            }
    ...
    private static void spawnNaturePulse(ServerLevel level, Vec3 pos, double radius) {
        for (int angle = 0; angle < 360; angle += 10) {
            ...
            level.sendParticles(new DustParticleOptions(dustColor(SMOG_COLOR), 2.5f), x, pos.y + 0.08, z, 0, vx, 0.02, vz, 1.0);
            if (RANDOM.nextFloat() < 0.3f) {
                level.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0xFFFFFFFF), x, pos.y + 0.15, z, 0, 0.2, 0.9, 0.2, 1.0);
            }
        ...
        for (int i = 0; i < inner; i++) {
            ...
            level.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0xFFFFFFFF), pos.x + Math.cos(a) * d, pos.y + 0.1, pos.z + Math.sin(a) * d, 0, 0.2, 0.9, 0.2, 1.0);
        }
    }
    ...
    private static void spawnNatureFade(ServerLevel level, Vec3 pos, double radius) {
        ...
        if (RANDOM.nextFloat() < 0.2f) {
            double a = RANDOM.nextDouble() * Math.PI * 2;
            double d = RANDOM.nextDouble() * radius;
            level.sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0xFFFFFFFF), pos.x + Math.cos(a) * d, pos.y + 0.2, pos.z + Math.sin(a) * d, 0, 0.2, 0.9, 0.2, 1.0);
        }
    }
    ...
    public static void playStaticChargedCloudTick(ServerLevel level, AreaEffectCloud cloud) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (RANDOM.nextFloat() < 0.2f) {
                level.sendParticles(new DustParticleOptions(dustColor(STATIC_PURPLE_BLUE), 1.0f),
                        x, y, z, 1, 0, 0, 0, 0);
            }
        }
    }
    ...
    public static void playStaticShockParticles(ServerLevel level, LivingEntity entity) {
        ...
        for (int i = 0; i < 2 + RANDOM.nextInt(2); i++) {
            ...
            if (RANDOM.nextFloat() < 0.125f) {
                ...
                level.sendParticles(new DustParticleOptions(dustColor(STATIC_PURPLE_BLUE), 1.2f),
                        x, y, z, 1, 0, 0, 0, 0);
            }
        }
    }
    ...
    private static int dustColor(org.joml.Vector3f color) {
        return (Math.round(color.x() * 255.0F) << 16)
