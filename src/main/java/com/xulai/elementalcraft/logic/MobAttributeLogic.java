...
@EventBusSubscriber(modid = com.xulai.elementalcraft.ElementalCraft.MODID)
public class MobAttributeLogic {
    ...
    private static void applyRandomAttributes(Mob mob) {
        ...
        if (attackType != null) {
            if (hasHandItem) {
                ...
                boolean piglin = BuiltInRegistries.ENTITY_TYPE.getKey(mob.getType()).toString().equals("minecraft:piglin");
    ...
    public static void processFlee(LivingEntity target, LivingEntity source, double range) {
        ...
        int fleeDist = 12 + target.level().getRandom().nextInt(9);
    ...
    public static void processFlee(LivingEntity target, double fleeFromX, double fleeFromZ, double range) {
        ...
        double dist = Math.sqrt(dx * dx + dz * dz);
        if (dist < 0.01) {
            dx = pfMob.level().getRandom().nextFloat() - 0.5;
            dz = pfMob.level().getRandom().nextFloat() - 0.5;
    ...
    private static boolean hasAuraEntityNearby(LivingEntity entity, double range) {
        ...
        for (LivingEntity le : nearby) {
            ...
            if (ScorchedHandler.isScorched(le) || FrostbiteHandler.hasFrostbite(le)
                    || le.getPersistentData().getIntOr(StaticShockHandler.NBT_STATIC_STACKS, 0) > 0) {
                return true;
            }
        }
        return false;
    ...
    public static void tickFlee(LivingEntity entity) {
        ...
        String sourceUUID = data.getStringOr(NBT_FLEE_SOURCE_UUID, "");
        double auraRange = data.getDoubleOr(NBT_FLEE_SOURCE_RANGE, 0.0);
        LivingEntity source = findSourceEntity(entity, sourceUUID);

        double targetX = data.getDoubleOr(NBT_FLEE_TARGET_X, 0.0);
        double targetZ = data.getDoubleOr(NBT_FLEE_TARGET_Z, 0.0);
        ...
        if (dist < 1.0) {
            ...
            int fleeDist = 12 + entity.level().getRandom().nextInt(9);
            double[] path;
        ...
        double lastX = data.getDoubleOr(NBT_FLEE_LAST_X, 0.0);
        double lastZ = data.getDoubleOr(NBT_FLEE_LAST_Z, 0.0);
        double movedXZ = Math.abs(entity.getX() - lastX) + Math.abs(entity.getZ() - lastZ);
        if (movedXZ < 0.05) {
            int stuckTicks = data.getIntOr(NBT_FLEE_STUCK_TICKS, 0) + 1;
    ...
    public static void onChangeTarget(LivingChangeTargetEvent event) {
        ...
        if (data.getIntOr(NBT_DISORIENTED, 0) > 0) {
    ...
    public static void onLivingTick(EntityTickEvent.Post event) {
        ...
        int disoriented = data.getIntOr(NBT_DISORIENTED, 0);
        if (disoriented > 0) {
