...
public class ThunderVisuals {
        ...
        synchronized (ACTIVE_PROJECTILES) {
            ...
            while (iterator.hasNext()) {
                ...
                if (enabled && p.level() instanceof ServerLevel serverLevel) {
                    ...
                    int tier = data.getIntOr(TAG_PROJECTILE_TIER, 0);
    ...
    private static void spawnHelixTrail(ServerLevel level, Projectile p, int tier) {
        ...
        CompoundTag data = p.getPersistentData();
        if (data == null) return;
        int spawnTick = data.getIntOr(TAG_SPAWN_TICK, 0);
    ...
    public static void onLivingDamage(LivingDamageEvent.Pre event) {
        ...
        if (directEntity instanceof Projectile projectile) {
            isRanged = true;
            CompoundTag data = projectile.getPersistentData();
            if (data != null && data.contains(TAG_ELEMENTAL_PROJECTILE) && data.getStringOr(TAG_ELEMENTAL_PROJECTILE, "").equals("thunder")) {
                isThunder = true;
                tier = data.getIntOr(TAG_PROJECTILE_TIER, 0);
            }
        } else {
