...
public class ReactionHandler {
    ...
    private static void handleReinfection(LivingEntity entity) {
        ...
        boolean hasSpores = ModMobEffects.SPORES.isBound()
                && entity.hasEffect(ModMobEffects.SPORES);
        if (hasSpores) return;
        if (data.getBooleanOr(NBT_INFECTED, false)) {
    ...
    private static void handleParalysisSporeBlast(LivingEntity entity) {
        ...
        if (!ModMobEffects.SPORES.isBound()
                || !entity.hasEffect(ModMobEffects.SPORES)) return;
    ...
    private static void handleScorchedSporeBlast(LivingEntity entity) {
        ...
        MobEffectInstance sporeEffect = entity.getEffect(ModMobEffects.SPORES);
    ...
    private static void handleEnvironmentalBlast(LivingEntity entity) {
        ...
        if (!triggered && entity.isOnFire() && !entity.getPersistentData().getBooleanOr(NBT_WAS_ON_FIRE, false)) {
    ...
    private static void handleContagionSpread(LivingEntity entity) {
        if (!ModMobEffects.SPORES.isBound() || !entity.hasEffect(ModMobEffects.SPORES)) return;
        MobEffectInstance sporeEffect = entity.getEffect(ModMobEffects.SPORES);
        if (sporeEffect == null) return;
    ...
    public static void onLivingDamage(LivingDamageEvent.Pre event) {
        ...
        if (attackType == ElementType.NATURE) {
            if (ElementalFireNatureReactionsConfig.natureParasiteBaseThreshold > 0
                ...
                double stackingBonus = target.hasEffect(ModMobEffects.SPORES)
    ...
    public static void onTryNatureCounter(LivingDamageEvent.Pre event) {
        if (event.getEntity().level().isClientSide()) return;
    ...
    public static SporeApplyResult stackSporeEffect(LivingEntity target, int layersToAdd, LivingEntity applier) {
        if (!ModMobEffects.SPORES.isBound()) return SporeApplyResult.NOT_REGISTERED;
        ...
        if (ElementalThunderFrostReactionsConfig.freezeClearSporesEnabled && target.hasEffect(ModMobEffects.FREEZE)) {
            return SporeApplyResult.FROZEN;
        }

        MobEffectInstance currentEffect = target.getEffect(ModMobEffects.SPORES);
    ...
    private static void processContagion(LivingEntity source, int stacks) {
        ...
        boolean isInfected = data.getBooleanOr(NBT_INFECTED, false);
        ...
        if (data.contains(NBT_CONTAGION_SOURCE)) {
            try {
                contagionSourceUUID = UUID.fromString(data.getStringOr(NBT_CONTAGION_SOURCE, ""));
        ...
        for (LivingEntity target : allNearby) {
            ...
            if (ModMobEffects.SPORES.isBound() && target.hasEffect(ModMobEffects.SPORES)) {
                continue;
            }
            ...
            if (onlyHostile) {
                ...
                if (target instanceof OwnableEntity ownable && ownable.getOwnerReference() != null) continue;
                if (target instanceof AbstractHorse horse && horse.getOwnerReference() != null) continue;
            }

            var entityKey = BuiltInRegistries.ENTITY_TYPE.getKey(target.getType());
        ...
        for (LivingEntity target : validTargets) {
            ...
            targetData.putString(NBT_CONTAGION_SOURCE, source.getUUID().toString());
    ...
    private static void triggerToxicBlast(Level level, LivingEntity attacker, LivingEntity target, double firePower, LivingEntity killCredit, int minStacks, Set<LivingEntity> visited) {
        ...
        if (!ModMobEffects.SPORES.isBound()) return;
        ...
        if (effectiveStacks < ElementalFireNatureReactionsConfig.sporeReactionThreshold) {
            ...
            level.playSound(null, target.getX(), target.getY(), target.getZ(), SoundEvents.GENERIC_EXPLODE, SoundSource.BLOCKS, 4.0F, (1.0F + (level.getRandom().nextFloat() - level.getRandom().nextFloat()) * 0.2F) * 0.7F);
            ...
            if (level instanceof ServerLevel serverLevel) {
                ...
                serverLevel.getServer().execute(() -> {
                    ...
                    for (LivingEntity entity : nearbyEntities) {
                        ...
                        affectedCount++;
                        if (ModMobEffects.SPORES.isBound()) {
                            MobEffectInstance entitySpore = entity.getEffect(ModMobEffects.SPORES);
                    ...
                    blastCtx.blastProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.BLAST_PROTECTION), target);
                    blastCtx.generalProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROTECTION), target);
    ...
    private static float calculateBlastMitigation(LivingEntity entity) {
        int blastProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.BLAST_PROTECTION), entity);
        int generalProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROTECTION), entity);
    ...
    public static boolean checkHealthRecovery(LivingEntity entity, String key) {
        ...
        double thresholdHealth = data.getDoubleOr(key, 0.0);
    ...
    public static void checkHeatSporeBlast(LivingEntity entity) {
        ...
        CompoundTag data = entity.getPersistentData();
        long cd = data.getLongOr(NBT_SPORE_HEAT_BLAST_CD, 0L);
