...
public class ElementalBookDropHandler {
    ...
    public static void onLivingDrops(LivingDropsEvent event) {
        ...
        if (event.getSource().getEntity() instanceof LivingEntity killer) {
            lootingLevel = EnchantmentHelper.getEnchantmentLevel(ModEnchantments.vanilla(Enchantments.LOOTING), killer);
        }
        double dropChance = ElementalConfig.enchantedBookDropChance + lootingLevel * ElementalConfig.enchantedBookLootingBonus;
        if (entity.level().getRandom().nextDouble() >= dropChance) return;

        String elementId = data.getStringOr(NBT_DROP_ELEMENT, "");
        ElementType elementType = ElementType.fromId(elementId);

        String attackTypeId = data.getStringOr(NBT_DROP_ATTACK_TYPE, "");
        ElementType attackType = ElementType.fromId(attackTypeId);

        String resistTypeId = data.contains(NBT_DROP_RESIST_TYPE)
                ? data.getStringOr(NBT_DROP_RESIST_TYPE, "")
                : elementId;
        ElementType resistType = ElementType.fromId(resistTypeId);

        int enhancePoints = data.getIntOr(NBT_DROP_ENHANCE_POINTS, 0);
        int resistPoints = data.getIntOr(NBT_DROP_RESIST_POINTS, 0);
        ...
        BookCandidate chosen = candidates.get(entity.level().getRandom().nextInt(candidates.size()));
    ...
    private static Holder<Enchantment> getEnchantment(BookType type, ElementType element) {
        return switch (type) {
