...
public class FireSpellHandler {
    ...
    public static void onFireSpellHit(LivingDamageEvent.Pre event) {
    ...
    public static void onBlazeStormHit(LivingIncomingDamageEvent event) {
        if (event.getEntity().level().isClientSide) return;
        if (!ISSCore.ISS_LOADED) return;
    ...
    public static void onRaiseHellHit(LivingDamageEvent.Pre event) {
        if (event.getEntity().level().isClientSide) return;
        if (!ISSCore.ISS_LOADED) return;
        if (!(event.getSource().getEntity() instanceof LivingEntity attacker)) return;
    ...
    @SubscribeEvent(priority = EventPriority.LOW)
    public static void onFlamingStrikeHit(LivingDamageEvent.Pre event) {
        if (event.getEntity().level().isClientSide) return;
        if (!ISSCore.ISS_LOADED) return;
        if (!(event.getSource().getEntity() instanceof LivingEntity attacker)) return;
        String lastSpell = attacker.getPersistentData().getString("EC_LastSpellId");
    ...
    @SubscribeEvent(priority = EventPriority.LOWEST)
    public static void onFireFieldHit(LivingDamageEvent.Pre event) {
        if (event.getEntity().level().isClientSide) return;
        if (!ISSCore.ISS_LOADED) return;
    ...
    @SubscribeEvent(priority = EventPriority.HIGHEST)
    public static void onAfterFireFieldTick(EntityTickEvent.Post event) {
