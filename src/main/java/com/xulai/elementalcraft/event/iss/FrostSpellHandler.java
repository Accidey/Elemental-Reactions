...
public class FrostSpellHandler {
    ...
    private static net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> chilledEffect() {
        return BuiltInRegistries.MOB_EFFECT.getHolder(ResourceLocation.fromNamespaceAndPath("irons_spellbooks", "chilled")).orElse(null);
    }

    @SubscribeEvent(priority = EventPriority.HIGHEST)
    public static void onIceSpellHit(LivingDamageEvent.Pre event) {
    ...
    public static void onIceSpellRefreshFreeze(LivingDamageEvent.Pre event) {
        if (event.getEntity().level().isClientSide) return;
        if (!ISSCore.ISS_LOADED) return;
        if (!isIceSpellDamage(event.getSource())) return;
        ...
        target.addEffect(new MobEffectInstance(
                ModMobEffects.FREEZE, freezeDuration, amplifier, false, true, true));
    ...
    @SubscribeEvent
    public static void onBlizzardAoeTick(LevelTickEvent.Post event) {
    ...
    @SubscribeEvent
    public static void onChilledWetnessFreeze(EntityTickEvent.Post event) {
        if (!ISSCore.ISS_LOADED) return;
        net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> chilled = chilledEffect();
        ...
        entity.addEffect(new MobEffectInstance(
                ModMobEffects.FREEZE, freezeDuration, freezeStacks - 1, false, true, true));
        data.putInt(FrostbiteHandler.NBT_FREEZE_STACKS, freezeStacks);
