...
public class ScorchedHandler {
    ...
    public static ScorchedApplyResult applyScorched(LivingEntity target, LivingEntity attacker, int fireStrength, int duration, int sourceFirePower, float damageMultiplier, boolean bypassCooldown) {
        ...
        if (targetData.contains(NBT_SCORCHED_TICKS) && targetData.getIntOr(NBT_SCORCHED_TICKS, 0) > 0) {
            if (attacker != null) {
        ...
        if (attacker != null) {
            targetData.putString(NBT_SCORCHED_ATTACKER, attacker.getUUID().toString());
        }
        ...
        if (target.level() instanceof ServerLevel serverLevel) {
            serverLevel.sendParticles(ParticleTypes.FLAME, target.getX(), target.getY() + target.getBbHeight() * 0.5, target.getZ(), 20, 0.2, 0.2, 0.2, 0.0);
        }
    ...
    public static boolean isScorched(LivingEntity entity) {
        ...
        return (data.contains(NBT_SCORCHED_TICKS) && data.getIntOr(NBT_SCORCHED_TICKS, 0) > 0)
                || data.getBooleanOr(NBT_TEMP_SCORCH, false);
    }

    public static UUID getScorchedAttackerUUID(LivingEntity entity) {
        CompoundTag data = entity.getPersistentData();
        if (data.contains(NBT_SCORCHED_ATTACKER)) {
            try {
                return UUID.fromString(data.getStringOr(NBT_SCORCHED_ATTACKER, ""));
    ...
    public static void onExplosionStart(ExplosionEvent.Start event) {
        ...
        if (!creeper.getPersistentData().getBooleanOr(NBT_CREEPER_POISON_ENHANCED, false)) return;
    ...
    public static boolean isTempScorched(LivingEntity entity) {
        CompoundTag data = entity.getPersistentData();
        return data.getBooleanOr(NBT_TEMP_SCORCH, false);
    }

    public static int getTempScorchFireStrength(LivingEntity entity) {
        return entity.getPersistentData().getIntOr(NBT_TEMP_SCORCH_STRENGTH, 0);
    }

    public static int getScorchFireStrength(LivingEntity entity) {
        CompoundTag data = entity.getPersistentData();
        if (data.contains(NBT_SCORCHED_TICKS) && data.getIntOr(NBT_SCORCHED_TICKS, 0) > 0) {
            return data.getIntOr(NBT_SCORCHED_SOURCE_FIRE_POWER, 0);
        }
        if (data.getBooleanOr(NBT_TEMP_SCORCH, false)) {
            return data.getIntOr(NBT_TEMP_SCORCH_STRENGTH, 0);
        }
        return 0;
    }
    ...
    public static void onLivingTick(EntityTickEvent.Post event) {
        ...
        if (data.contains(NBT_ATTACKER_SCORCHED_COOLDOWN)) {
            long cd = data.getLongOr(NBT_ATTACKER_SCORCHED_COOLDOWN, 0L);
    ...
    private static void handleTempScorch(LivingEntity entity, CompoundTag data) {
        if (!data.getBooleanOr(NBT_TEMP_SCORCH, false)) return;
        int ttl = data.getIntOr(NBT_TEMP_SCORCH_TTL, 0) - 1;
        if (ttl <= 0) {
        ...
        int tempStrength = data.getIntOr(NBT_TEMP_SCORCH_STRENGTH, 0);

        int steamCooldown = data.getIntOr(NBT_WETNESS_STEAM_COOLDOWN, 0);
        if (steamCooldown > 0) {
        ...
        if (entity.isInWater()) {
            ...
            triggerThermalShock(entity, sLevel, ttl, data.getIntOr(NBT_TEMP_SCORCH_STRENGTH, 0));
            clearTempScorch(entity);
            return;
        }
        ...
        if (entity.tickCount % 20 == 0) {
            int fs = data.getIntOr(NBT_TEMP_SCORCH_STRENGTH, 0);
    ...
    private static void handleRegularScorch(LivingEntity entity, CompoundTag data) {
        int ticks = data.getIntOr(NBT_SCORCHED_TICKS, 0);
        ...
        int fireStrength = data.getIntOr(NBT_SCORCHED_STRENGTH, 0);
        ...
        if (entity.tickCount % 20 == 0) {
            ...
            int sourceFirePower = data.getIntOr(NBT_SCORCHED_SOURCE_FIRE_POWER, 0);
    ...
    public static void onEffectApplicable(MobEffectEvent.Applicable event) {
        if (event.getEffectInstance().getEffect().value() == ModMobEffects.WETNESS.get()) {
    ...
    private static void applyAuraDamage(LivingEntity source, int sourceFirePower, int fireStrength, ServerLevel level) {
        ...
        for (LivingEntity target : nearby) {
            ...
            CompoundTag targetData = target.getPersistentData();
            if (targetData.contains(NBT_SCORCHED_TICKS) && targetData.getIntOr(NBT_SCORCHED_TICKS, 0) > 0) continue;
            if (targetData.getBooleanOr(NBT_FLEE_ACTIVE, false) && target instanceof Mob) continue;
            ...
            if (auraDamage > 0) {
                ...
                if (source.getPersistentData().getIntOr(NBT_SCORCHED_AURA_LOGGED, 0) == 0) {
                    float dmgMult = target.getPersistentData().getFloatOr(NBT_SCORCHED_DAMAGE_MULT, 0.0F);
                    if (dmgMult <= 0.0f) dmgMult = 1.0f;
                    String multSrc = target.getPersistentData().getStringOr(NBT_SCORCHED_DAMAGE_MULT_SRC, "");
    ...
    static ScorchedDamageResult calculateScorchedDamageDetailed(int fireStrength, LivingEntity target) {
        ...
        float damageMult = target.getPersistentData().getFloatOr(NBT_SCORCHED_DAMAGE_MULT, 0.0F);
    ...
    private static int[] getTotalEnchantmentLevels(LivingEntity entity) {
        ...
        for (ItemStack stack : java.util.List.of(entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.HEAD), entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.CHEST), entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.LEGS), entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.FEET))) {
            fireProt += stack.getEnchantmentLevel(ModEnchantments.vanilla(Enchantments.FIRE_PROTECTION));
            genProt += stack.getEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROTECTION));
        }
    ...
    private static boolean handleFrostbiteToWetness(LivingEntity entity, String debugKey) {
        ...
        int frostbiteStacks = hasReal
                ...
                : entity.getPersistentData().getIntOr(FrostbiteHandler.NBT_TEMP_FROSTBITE_STACKS, 0);
    ...
    private static void applyScorchedTickDamage(LivingEntity entity, float baseDamage, ElementType entityElement, float elementMult, ServerLevel level, boolean logTick, ScorchedDamageResult detail) {
        ...
        level.sendParticles(ParticleTypes.FLAME, entity.getX(), entity.getY() + entity.getBbHeight() * 0.5, entity.getZ(), 10, 0.2, 0.2, 0.2, 0.0);
        level.sendParticles(ParticleTypes.SMOKE, entity.getX(), entity.getY() + entity.getBbHeight() * 0.5, entity.getZ(), 5, 0.2, 0.2, 0.2, 0.0);
        if (logTick) {
            CompoundTag data = entity.getPersistentData();
            if (data.getIntOr(NBT_SCORCHED_TICK_LOGGED, 0) == 0) {
                level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.FIRE_EXTINGUISH, SoundSource.PLAYERS, 0.2f, 1.0f);
                float dmgMult = data.getFloatOr(NBT_SCORCHED_DAMAGE_MULT, 0.0F);
                if (dmgMult <= 0.0f) dmgMult = 1.0f;
                float rawBase = baseDamage / dmgMult;
                String multSrc = data.getStringOr(NBT_SCORCHED_DAMAGE_MULT_SRC, "");
                if (detail != null) {
    ...
    public static void onTryFireCounter(LivingDamageEvent.Pre event) {
    ...
    private static void doExplosion(ServerLevel level, ActiveFireCounter fc, LivingEntity owner) {
        ...
        for (UUID uid : fc.affectedEntities) {
            ...
            if (delta.lengthSqr() < 1e-7) {
                delta = new Vec3(level.getRandom().nextDouble() - 0.5, 0, level.getRandom().nextDouble() - 0.5);
            }
    ...
    private static void spawnFireRingParticles(ServerLevel level, double cx, double cy, double cz, double radius) {
        ...
        for (int i = 0; i < count; i += 2) {
            ...
            level.sendParticles(ParticleTypes.FLAME, px, cy + 0.1, pz, 1, 0, 0, 0, 0);
    ...
    private static void spawnFireTornado(ServerLevel level, Vec3 pos, double radius, int tick) {
        ...
        for (int h = 0; h < totalHelices; h++) {
            ...
            for (int p = 0; p < layers; p++) {
                ...
                double vx = -Math.sin(helixAngle) * (0.15 + 0.1 * level.getRandom().nextDouble())
                        + Math.cos(helixAngle) * (0.03 + 0.02 * level.getRandom().nextDouble());
                double vz = Math.cos(helixAngle) * (0.15 + 0.1 * level.getRandom().nextDouble())
                        + Math.sin(helixAngle) * (0.03 + 0.02 * level.getRandom().nextDouble());
                double vy = tick <= expandTicks ? 0.03 + 0.02 * level.getRandom().nextDouble() : 0;
                level.sendParticles(ParticleTypes.FLAME, x, pos.y + y, z, 0, vx, vy, vz, 1.0);
