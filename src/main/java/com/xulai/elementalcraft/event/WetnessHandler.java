...
public class WetnessHandler {
    ...
    public static int getWetnessLevel(LivingEntity entity) {
        ...
        if (data.contains(NBT_WETNESS)) {
            return data.getIntOr(NBT_WETNESS, 0);
        }
        MobEffectInstance effect = entity.getEffect(ModMobEffects.WETNESS);
        if (effect != null) {
    ...
    public static void onLivingTick(EntityTickEvent.Post event) {
        ...
        if (state.is(Blocks.FIRE) || state.is(Blocks.SOUL_FIRE)) {
            CompoundTag data = entity.getPersistentData();
            if (wetnessLevel > 0) {
                int timer = data.getIntOr(NBT_FIRE_STAND_TIMER, 0) + 1;
    ...
    private static void handleWetnessLogic(LivingEntity entity, boolean isSnowing) {
        ...
        if (inWater) {
            ...
            if (currentLevel < targetLevel && !blockWetnessIfParalyzed(entity) && !blockWetnessIfFrozen(entity)) {
                ...
                currentLevel = data.getIntOr(NBT_WETNESS, 0);
            }
            ...
            if (currentLevel < maxLevel) {
                int rainTimer = data.getIntOr(NBT_RAIN_TIMER, 0) + 1;
            ...
            if (currentLevel > 0) {
                ...
                float progress = data.getFloatOr(NBT_DECAY_PROGRESS, 0.0F) + (float) finalMult;
        ...
        if (currentLevel > 0 && !data.getBooleanOr(NBT_REACTION_RESOLVED, false)) {
            boolean hasStatic = data.getIntOr(StaticShockHandler.NBT_STATIC_STACKS, 0) > 0;
    ...
    private static boolean isSnowingHere(LivingEntity entity) {
        ...
        return biome != null && biome.getPrecipitationAt(pos, level.getSeaLevel()) == Biome.Precipitation.SNOW;
    }
    ...
    private static void migrateDecayNbt(CompoundTag data) {
        if (data.contains(NBT_DECAY_TIMER) && !data.contains(NBT_DECAY_PROGRESS)) {
            data.putFloat(NBT_DECAY_PROGRESS, data.getIntOr(NBT_DECAY_TIMER, 0));
    ...
    static void tryColdBiomeFreeze(LivingEntity entity, Level level, BlockPos pos) {
        ...
        CompoundTag data = entity.getPersistentData();
        long cooldownEnd = data.getLongOr(NBT_COLD_FREEZE_TIMER, 0L);
    ...
    private static boolean hasMaxedSpores(LivingEntity entity) {
        ...
        if (!ModMobEffects.SPORES.isBound()) return false;
        MobEffectInstance spores = entity.getEffect(ModMobEffects.SPORES);
    ...
    public static int convertWetnessToSpores(LivingEntity entity) {
        ...
        if (!ReactionHandler.isSporeImmune(entity)) {
            MobEffectInstance currentSpores = entity.getEffect(ModMobEffects.SPORES);
    ...
    private static void syncEffect(LivingEntity entity, int level, boolean isPaused) {
        ...
        if (ModMobEffects.SPORES.isBound() && entity.hasEffect(ModMobEffects.SPORES)) {
            boolean hasStatic = entity.getPersistentData().getIntOr(StaticShockHandler.NBT_STATIC_STACKS, 0) > 0;
            boolean hasFrostbite = FrostbiteHandler.hasFrostbite(entity) || FrostbiteHandler.isTempFrostbite(entity);
            if (!hasStatic && !hasFrostbite) {
        ...
        if (isPaused) {
            ...
            CompoundTag data = entity.getPersistentData();
            migrateDecayNbt(data);
            float progress = data.getFloatOr(NBT_DECAY_PROGRESS, 0.0F);
        ...
        if (durationTicks > 0) {
            MobEffectInstance existing = entity.getEffect(ModMobEffects.WETNESS);
    ...
    static boolean blockWetnessIfParalyzed(LivingEntity entity) {
        ...
        CompoundTag data = entity.getPersistentData();
        if (!data.getBooleanOr("EC_WetnessParalysisLogged", false)) {
    ...
    static boolean blockWetnessIfFrozen(LivingEntity entity) {
        ...
        CompoundTag data = entity.getPersistentData();
        if (!data.getBooleanOr("EC_WetnessFrozenLogged", false)) {
    ...
    private static void handleExhaustion(LivingEntity entity) {
        if (entity instanceof Player player && !player.isCreative() && !player.isSpectator()) {
            ...
            float currentExhaustion = player.getFoodData().exhaustionLevel;
            float lastExhaustion = data.getFloatOr(NBT_LAST_EXHAUSTION, 0.0F);
            float delta;
            ...
            if (delta > 0.0001f) {
                ...
                if (currentLevel > 0) {
                    ...
                    currentExhaustion = player.getFoodData().exhaustionLevel;
                }
                if (extra > 0) {
    ...
    public static void resolveElementReactionConflict(LivingEntity entity, LivingEntity attacker) {
        ...
        if (frostbiteStacks <= 0 && FrostbiteHandler.isTempFrostbite(entity)) {
            frostbiteStacks = data.getIntOr(FrostbiteHandler.NBT_TEMP_FROSTBITE_STACKS, 0);
        }
        boolean hasSpores = ModMobEffects.SPORES.isBound() && entity.hasEffect(ModMobEffects.SPORES);
        ...
        int frostPower = attacker != null
                ...
                : data.getIntOr(FrostbiteHandler.NBT_FROSTBITE_SOURCE_FROST_POWER, 0);
    ...
    public static void onMobEffectRemoved(MobEffectEvent.Remove event) {
        ...
        if (event.getEffect().value() != ModMobEffects.WETNESS.get()) return;
    ...
    public static void onMobEffectExpired(MobEffectEvent.Expired event) {
        if (event.getEffectInstance() == null || event.getEffectInstance().getEffect().value() != ModMobEffects.WETNESS.get()) return;
        LivingEntity entity = event.getEntity();
        if (entity.level().isClientSide()) return;
