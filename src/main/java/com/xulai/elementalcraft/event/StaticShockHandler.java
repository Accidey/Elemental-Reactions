...
public class StaticShockHandler {
    ...
    public static boolean blockStaticIfParalyzed(LivingEntity entity) {
        if (!entity.hasEffect(ModMobEffects.PARALYSIS)) return false;
    ...
    private static void trimStaticStacks(LivingEntity entity) {
        ...
        if (currentStacks > maxStacks) {
            int totalTimer = data.getIntOr(NBT_STATIC_TIMER, 0);
    ...
    public static void onLivingDamage(LivingDamageEvent.Pre event) {
        ...
        if (event.getSource().typeHolder().unwrapKey().isPresent()) {
            ...
            if ("irons_spellbooks".equals(key.identifier().getNamespace())
                    && key.identifier().getPath().endsWith("_magic")) return;
        }
        ...
        if (isImmuneToStatic(target)) {
            ...
            int targetStacks = data.getIntOr(NBT_STATIC_STACKS, 0);
        ...
        boolean hasWetness = target.hasEffect(ModMobEffects.WETNESS);
        int wetnessLevel = 0;
        if (hasWetness) {
            MobEffectInstance wetnessEffect = target.getEffect(ModMobEffects.WETNESS);
            if (wetnessEffect != null) {
                wetnessLevel = wetnessEffect.getAmplifier() + 1;
            }
        }

        double stackingBonus = target.hasEffect(ModMobEffects.STATIC_SHOCK)
        ...
        if (wetnessLevel > 0) {
            ...
            int newTotalTicks = data.getIntOr(NBT_STATIC_TIMER, 0) + addTicks;
        ...
        int durationPerStack = ElementalThunderFrostReactionsConfig.staticDurationPerStackTicks;
        int addTicks = actualAdded * durationPerStack;
        int currentTimer = data.getIntOr(NBT_STATIC_TIMER, 0);
    ...
    public static void onLivingTick(EntityTickEvent.Post event) {
        ...
        if (isImmuneToStatic(entity)) {
            CompoundTag data = entity.getPersistentData();
            int staticStacks = data.getIntOr(NBT_STATIC_STACKS, 0);
        ...
        CompoundTag data = entity.getPersistentData();
        int cooldownTimer = data.getIntOr(NBT_PARALYSIS_COOLDOWN_TIMER, 0);
        if (cooldownTimer > 0) {
            cooldownTimer--;
        ...
        MobEffectInstance effectInstance = entity.getEffect(ModMobEffects.STATIC_SHOCK);
        ...
        if (effectInstance != null && data.contains(NBT_STATIC_STACKS)) {
            if (effectInstance.getDuration() > data.getIntOr(NBT_STATIC_TIMER, 0)) {
        ...
        int stacks = data.getIntOr(NBT_STATIC_STACKS, 0);
        if (stacks <= 0) {
            clearStaticShock(entity);
            return;
        }
        ...
        boolean hasWetness = entity.hasEffect(ModMobEffects.WETNESS);
        int wetnessLevel = 0;
        if (hasWetness) {
            MobEffectInstance wetnessEffect = entity.getEffect(ModMobEffects.WETNESS);
            if (wetnessEffect != null) {
                wetnessLevel = wetnessEffect.getAmplifier() + 1;
            }
        }
        ...
        int totalTimer = data.getIntOr(NBT_STATIC_TIMER, 0);
        int damageTimer = data.getIntOr(NBT_STATIC_DAMAGE_TIMER, 0);
        ...
        if (elec != null) {
            if (now - elec.startTick >= elec.duration) {
                ...
                for (LivingEntity target : sl.getEntitiesOfClass(LivingEntity.class, area)) {
                    if (target.hasEffect(ModMobEffects.PARALYSIS)) {
        ...
        if (storm != null) {
            ...
            for (LivingEntity strikeTarget : areaEntities) {
                ...
                if (currentStacks < maxStacks) {
                    ...
                    int durationPerStack = ElementalThunderFrostReactionsConfig.staticDurationPerStackTicks;
                    int totalTimer = data.getIntOr(NBT_STATIC_TIMER, 0) + durationPerStack;
                    data.putInt(NBT_STATIC_STACKS, newStacks);
                    data.putInt(NBT_STATIC_TIMER, totalTimer);
    ...
    public static boolean tryTriggerWaterElectrification(LivingEntity source, int paralysisDuration) {
        int stacks = source.getPersistentData().getIntOr(NBT_STATIC_STACKS, 0);
    ...
    private static int targetInFrostAuraRange(LivingEntity target, LivingEntity exclude) {
        ...
        for (LivingEntity source : target.level().getEntitiesOfClass(LivingEntity.class, searchArea)) {
            ...
            int stacks = source.getPersistentData().getIntOr(FrostbiteHandler.NBT_FROSTBITE_STACKS, 0);
    ...
    private static void applyStaticAuraEffects(LivingEntity source, int stacks) {
        ...
        CompoundTag sourceData = source.getPersistentData();
        String trackedStr = sourceData.getStringOr(NBT_AURA_TRACKED, "");
        ...
        for (LivingEntity target : nearby) {
            ...
            int targetStaticStacks = target.getPersistentData().getIntOr(NBT_STATIC_STACKS, 0);
            if (targetStaticStacks <= 0) targetStaticStacks = target.getPersistentData().getIntOr(NBT_TEMP_STATIC_STACKS, 0);
            if (ElementalThunderFrostReactionsConfig.thunderBreakFreezeToWetnessRatio > 0
                    ...
                    && !target.getPersistentData().getBooleanOr(FrostbiteHandler.NBT_FROST_BURST_FROZEN, false)
                ...
                if (FrostbiteHandler.isFreezeImmune(target)) {
                    ...
                    if (!targetData.contains(NBT_THUNDER_BREAK_FREEZE_CD) || targetGameTime >= targetData.getLongOr(NBT_THUNDER_BREAK_FREEZE_CD, 0L)) {
            ...
            boolean targetHasWetness = target.hasEffect(ModMobEffects.WETNESS);
            boolean targetHasParalysis = target.hasEffect(ModMobEffects.PARALYSIS);
            if (targetHasWetness && !isImmuneToParalysis(target)) {
                boolean sourceHasFrostbiteAura = FrostbiteHandler.hasFrostbite(source)
                        ...
                        && source.getPersistentData().getIntOr(FrostbiteHandler.NBT_FROSTBITE_STACKS, 0) >= ElementalThunderFrostReactionsConfig.frostbiteAuraThreshold;
                if (sourceHasFrostbiteAura) {
                    int frostbiteStacks = source.getPersistentData().getIntOr(FrostbiteHandler.NBT_FROSTBITE_STACKS, 0);
            ...
            if (sporesEffect != null && target.hasEffect(sporesEffect)) {
                ...
                if (targetData.contains(NBT_AURA_SPORE_CD) && targetData.getLongOr(NBT_AURA_SPORE_CD, 0L) > gameTime) continue;
                ...
                int sourceStacks = source.getPersistentData().getIntOr(NBT_STATIC_STACKS, 0);
    ...
    private static void applyStaticAuraDamage(LivingEntity source, int stacks, boolean shouldDamage) {
        ...
        for (LivingEntity target : nearby) {
            ...
            if (isImmuneToStatic(target)) continue;
            if (target.getPersistentData().getBooleanOr("EC_FleeActive", false) && target instanceof Mob) continue;
        ...
        int syncPhase = sourceData.getIntOr(NBT_AURA_SYNC_PHASE, 0);
        if (syncPhase > 0 && source.level() instanceof ServerLevel sl) {
            float lastDamage = sourceData.getFloatOr(NBT_LAST_STATIC_DAMAGE, 0.0F);
            if (syncPhase == 1) {
                if (lastDamage > 0) {
                    for (LivingEntity target : nearby) {
                ...
                if (lastDamage > 0) {
                    float lastLoggedDamage = sourceData.getFloatOr(NBT_LAST_AURA_LOG_DAMAGE, 0.0F);
                    boolean damageChanged = lastLoggedDamage != lastDamage;
                    float auraBaseDamage = sourceData.getFloatOr(NBT_LAST_STATIC_BASE_DAMAGE, 0.0F);
                    float auraElementMult = sourceData.getFloatOr(NBT_LAST_STATIC_ELEMENT_MULT, 0.0F);
                    if (auraElementMult == 0) auraElementMult = 1.0f;
                    ElementType auraElement = ElementType.fromId(sourceData.getStringOr(NBT_LAST_STATIC_ELEMENT, ""));
                    ...
                    for (LivingEntity target : nearby) {
                        ...
                        if (targetDamage > 0) {
                            ...
                            if (damageChanged) {
                                ...
                                int tProt = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROTECTION), target);
                                int tProj = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROJECTILE_PROTECTION), target);
    ...
    private static float applyEnchantmentReduction(LivingEntity entity, float damage) {
        int totalProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROTECTION), entity);
        int totalProjectileProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROJECTILE_PROTECTION), entity);
    ...
    private static float triggerStaticDamage(LivingEntity entity) {
        ...
        int protLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROTECTION), entity);
        int projProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROJECTILE_PROTECTION), entity);
    ...
    private static int getTotalEnchantmentLevel(Holder<net.minecraft.world.item.enchantment.Enchantment> ench, LivingEntity entity) {
        int total = 0;
        for (ItemStack stack : java.util.List.of(entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.HEAD), entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.CHEST), entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.LEGS), entity.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.FEET))) {
    ...
    private static void tryPrimingOrIgniteCreeper(LivingEntity target) {
        ...
        boolean alreadyPrimed = data.getBooleanOr(NBT_STATIC_PRIMED, false);
        ...
        if (isLightningCharged) {
            ...
            if (creeper.level() instanceof ServerLevel sl) {
                sl.sendParticles(net.minecraft.core.particles.ColorParticleOption.create(ParticleTypes.FLASH, 0xFFFFFFFF),
            ...
            if (creeper.level() instanceof ServerLevel sl) {
                net.minecraft.world.entity.LightningBolt lightning =
                    net.minecraft.world.entity.EntityTypes.LIGHTNING_BOLT.create(sl, net.minecraft.world.entity.EntitySpawnReason.COMMAND);
                if (lightning != null) {
    ...
    public static void tryTriggerSporeBlast(LivingEntity target) {
        if (!target.hasEffect(ModMobEffects.SPORES)) return;
        ...
        CompoundTag data = target.getPersistentData();
        int staticStacks = data.getIntOr(NBT_STATIC_STACKS, 0);
        if (staticStacks <= 0 && data.getBooleanOr(NBT_TEMP_STATIC, false)) {
            staticStacks = data.getIntOr(NBT_TEMP_STATIC_STACKS, 0);
        }
    ...
    private static boolean tryBreakFreeze(LivingEntity entity, int staticStacks) {
        if (ElementalThunderFrostReactionsConfig.thunderBreakFreezeToWetnessRatio <= 0
                ...
                || entity.getPersistentData().getBooleanOr(FrostbiteHandler.NBT_FROST_BURST_FROZEN, false)) {
            return false;
        }
        ...
        if (data.contains(NBT_THUNDER_BREAK_FREEZE_CD) && gameTime < data.getLongOr(NBT_THUNDER_BREAK_FREEZE_CD, 0L)) {
            return false;
        }
        ...
        if (!WetnessHandler.blockWetnessIfParalyzed(entity) && !SteamReactionHandler.isInCondensingCloud(entity)) {
            ...
            if (entity.hasEffect(ModMobEffects.WETNESS)) {
                entity.removeEffect(ModMobEffects.WETNESS);
            }
            entity.addEffect(new MobEffectInstance(
                    ModMobEffects.WETNESS,
                    layers * 200,
    ...
    private static void updateEffect(LivingEntity entity, int stacks, int totalTicks) {
        ...
        MobEffectInstance currentEffect = entity.getEffect(ModMobEffects.STATIC_SHOCK);
    ...
    public static void onTryThunderCounter(LivingDamageEvent.Pre event) {
        if (event.getEntity().level().isClientSide()) return;
        ...
        if (currentHP - event.getNewDamage() >= target.getMaxHealth() * bloodThreshold) return;
    ...
    public static void triggerParalysisReaction(LivingEntity attacker, LivingEntity entity) {
        ...
        if (isImmuneToStatic(entity)) {
            CompoundTag data = entity.getPersistentData();
            int auraStacks = data.getIntOr(NBT_STATIC_STACKS, 0);
        ...
        int cooldownRemaining = data.getIntOr(NBT_PARALYSIS_COOLDOWN_TIMER, 0);
