...
public class SteamReactionHandler {
    ...
    public static void onLivingTick(EntityTickEvent.Post event) {
        ...
        if (data.contains(NBT_STEAM_ATTACKER_COOLDOWN)) {
            int cooldown = data.getIntOr(NBT_STEAM_ATTACKER_COOLDOWN, 0);
            if (cooldown > 1) {
    ...
    public static void onLevelTick(LevelTickEvent.Post event) {
        ...
        if (event.getLevel().getGameTime() % 10 != 0) return;
        ...
        for (AreaEffectCloud cloud : ACTIVE_STEAM_CLOUDS) {
            if (cloud.level() == event.getLevel()) {
                boolean isHighHeat = cloud.entityTags().contains(TAG_HIGH_HEAT);
                boolean isStaticCharged = cloud.entityTags().contains(TAG_STATIC_CHARGED);
                boolean isFrosted = cloud.entityTags().contains(TAG_FROSTED);
                EffectHelper.playSteamCloudTick((ServerLevel) event.getLevel(), cloud, isHighHeat);
                if (isFrosted) {
                    EffectHelper.playFrostedCloudTick((ServerLevel) event.getLevel(), cloud);
                } else if (isStaticCharged) {
                    EffectHelper.playStaticChargedCloudTick((ServerLevel) event.getLevel(), cloud);
                }
            }
        }
    }
    ...
    public static boolean isInCondensingCloud(LivingEntity entity) {
        ...
        List<AreaEffectCloud> clouds = entity.level().getEntitiesOfClass(AreaEffectCloud.class, box, c -> c.entityTags().contains(TAG_STEAM_CLOUD) && !c.entityTags().contains(TAG_HIGH_HEAT));
    ...
    private static void processTriggerLogic(LivingDamageEvent.Pre event, LivingEntity attacker, LivingEntity target) {
        if (target.getPersistentData().getBooleanOr(NBT_FIRE_FROST_MELT_RESOLVED, false)) {
    ...
    private static void processCloudEffects(LivingEntity entity) {
        ...
        for (AreaEffectCloud cloud : clouds) {
            ...
            if (cloud.entityTags().contains(TAG_HIGH_HEAT)) {
                isHighHeat = true;
                heatSource = cloud;
                ...
                isCondensing = true;
                if (cloud.entityTags().contains(TAG_STATIC_CHARGED)) {
                    isStaticCharged = true;
                }
                if (cloud.entityTags().contains(TAG_FROSTED)) {
                    isFrosted = true;
                }
            }
        }
        ...
        int auraStaticStacks = entity.getPersistentData().getIntOr(StaticShockHandler.NBT_STATIC_STACKS, 0);
        ...
        if (auraFrostStacks <= 0 && FrostbiteHandler.isTempFrostbite(entity)) {
            auraFrostStacks = entity.getPersistentData().getIntOr(FrostbiteHandler.NBT_TEMP_FROSTBITE_STACKS, 0);
        }
        ...
        if ((hasStaticAura || hasFrostAura) && !clouds.isEmpty()) {
            ...
            for (AreaEffectCloud cloud : clouds) {
                if (cloud.entityTags().contains(TAG_HIGH_HEAT)) continue;
                if (cloud.entityTags().contains(TAG_STATIC_CHARGED) || cloud.entityTags().contains(TAG_FROSTED)) continue;
                ...
                if (convertToStatic
                    ...
                    int sourceTimer = staticData.getIntOr(NBT_STATIC_TIMER, 0);
        ...
        for (AreaEffectCloud c : clouds) {
            if (c.entityTags().contains(TAG_FROST_OWNER_PREFIX + entity.getUUID())) {
                isFrostOwner = true;
                break;
            }
        }
        ...
        if (isHighHeat) {
            ...
            if (entity.tickCount % 20 == 0 && !aboveCeiling) {
                ...
                int fireProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.FIRE_PROTECTION), entity);
                int genProtLevel = getTotalEnchantmentLevel(ModEnchantments.vanilla(Enchantments.PROTECTION), entity);
                double denom = Math.max(1.0, ElementalFireNatureReactionsConfig.enchantmentCalculationDenominator);
                double maxFireCap = ElementalFireNatureReactionsConfig.steamMaxFireProtCap;
                double maxGeneralCap = ElementalFireNatureReactionsConfig.steamMaxGeneralProtCap;
                ...
                if (damage > 0 && !checkImmunity(entity)) {
                    ...
                    if (!entity.getPersistentData().getBooleanOr(NBT_STEAM_SCALDING_LOGGED, false)) {
            ...
            if (FrostbiteHandler.isFrozen(entity) && !entity.getPersistentData().getBooleanOr(FrostbiteHandler.NBT_FROST_BURST_FROZEN, false)) {
            ...
            if (!isStaticCharged && ElementalThunderFrostReactionsConfig.staticSteamCloudTriggerStacks > 0) {
                CompoundTag data = entity.getPersistentData();
                int staticStacks = data.getIntOr(StaticShockHandler.NBT_STATIC_STACKS, 0);
                if (staticStacks >= ElementalThunderFrostReactionsConfig.staticSteamCloudTriggerStacks) {
                    int sourceTimer = data.getIntOr(NBT_STATIC_TIMER, 0);
                    int interval = ElementalThunderFrostReactionsConfig.staticDamageIntervalTicks;
                    ...
                    if (anyCharged) {
                        ...
                        int cloudDuration = 0;
                        for (AreaEffectCloud cloud : clouds) {
                            if (isEntityInCloud(entity, cloud) && cloud.entityTags().contains(TAG_STATIC_CHARGED)) {
            ...
            if (isStaticCharged && ElementalThunderFrostReactionsConfig.staticSteamCloudTriggerStacks > 0) {
                ...
                if (isImmuneToThunderResist(entity)) {
                    ...
                    String appliedDmgUUID = entity.getPersistentData().getStringOr(NBT_STATIC_DMG_CLOUD_UUID, "");
                    ...
                    if (!isImmuneToParalysis(entity)) {
                        ...
                        if (paralysisDuration > 0) {
                            ...
                            MobEffectInstance currentParalysis = entity.getEffect(ModMobEffects.PARALYSIS);
            ...
            if (!isFrosted && ElementalThunderFrostReactionsConfig.frostedSteamCloudTriggerStacks > 0) {
                ...
                if (frostbiteStacks <= 0 && FrostbiteHandler.isTempFrostbite(entity)) {
                    frostbiteStacks = entity.getPersistentData().getIntOr(FrostbiteHandler.NBT_TEMP_FROSTBITE_STACKS, 0);
                }
                if (frostbiteStacks >= ElementalThunderFrostReactionsConfig.frostedSteamCloudTriggerStacks || FrostbiteHandler.isFrozen(entity)
                    ...
                    for (AreaEffectCloud cloud : clouds) {
                        if (!isEntityInCloud(entity, cloud)) continue;
                        if (!cloud.entityTags().contains(TAG_HIGH_HEAT) && !cloud.entityTags().contains(TAG_FROSTED)) {
                            cloud.addTag(TAG_FROSTED);
                        }
                    }
                    isFrosted = true;
                    ...
                    for (AreaEffectCloud cloud : clouds) {
                        if (isEntityInCloud(entity, cloud) && cloud.entityTags().contains(TAG_FROSTED)) {
            ...
            if (isFrosted && !FrostbiteHandler.isFrostbiteImmune(entity) && !FrostbiteHandler.isFreezeImmune(entity)) {
                ...
                for (AreaEffectCloud c : clouds) {
                    if (c.entityTags().contains(TAG_FROSTED) && c.entityTags().contains(TAG_FROST_OWNER_PREFIX + entity.getUUID())) {
                ...
                if (!isFrostCounterOwner) {
                    ...
                    if (maxStacks > 0) {
                        ...
                        entity.addEffect(new MobEffectInstance(ModMobEffects.FREEZE, freezeDuration, freezeStacks - 1, false, true, true));
            ...
            if (entity.getPersistentData().contains(NBT_FROSTED_CLOUD_UUID)) {
                String savedUUID = entity.getPersistentData().getStringOr(NBT_FROSTED_CLOUD_UUID, "");
                AreaEffectCloud matchedCloud = null;
                for (AreaEffectCloud cloud : clouds) {
                    if (cloud.entityTags().contains(TAG_FROSTED) && isEntityInCloud(entity, cloud)) {
            ...
            if (!isStaticCharged && !isFrosted) {
                int currentTimer = entity.getPersistentData().getIntOr(NBT_CONDENSATION_TIMER, 0);
                currentTimer += 10;
    ...
    private static List<AreaEffectCloud> findNearbyClouds(LivingEntity entity) {
        ...
        List<AreaEffectCloud> clouds = entity.level().getEntitiesOfClass(AreaEffectCloud.class, box, c -> c.entityTags().contains(TAG_STEAM_CLOUD));

        String frostedUUID = entity.getPersistentData().getStringOr(NBT_FROSTED_CLOUD_UUID, "");
        if (!frostedUUID.isEmpty()) {
            for (AreaEffectCloud c : ACTIVE_STEAM_CLOUDS) {
                if (!c.isRemoved() && c.level() == entity.level() && c.entityTags().contains(TAG_STEAM_CLOUD) && getCloudUUID(c).equals(frostedUUID) && !clouds.contains(c)) {
                    clouds.add(c);
                    break;
                }
            }
    ...
    private static void handleEmptyClouds(LivingEntity entity) {
        ...
        if (data.contains(NBT_FROSTED_CLOUD_UUID)) {
            boolean hasBurst = data.getBooleanOr(FrostbiteHandler.NBT_FROST_BURST_FROZEN, false);
            if (!hasBurst) {
                entity.removeEffect(ModMobEffects.FREEZE);
    ...
    public static void discardFrostedCloudsNear(LivingEntity target) {
        ...
        for (AreaEffectCloud cloud : ACTIVE_STEAM_CLOUDS) {
            ...
            if (!cloud.entityTags().contains(TAG_FROSTED)) continue;
    ...
    private static AreaEffectCloud spawnSteamCloud(ServerLevel serverLevel, double x, double y, double z, float height, boolean isHighHeat, int fuelLevel) {
        ...
        cloud.setCustomParticle(new BlockParticleOption(ParticleTypes.BLOCK, Blocks.AIR.defaultBlockState()));
    ...
    private static String getFirstCloudUUIDWithTag(List<AreaEffectCloud> clouds, LivingEntity entity, String requiredTag) {
        for (AreaEffectCloud cloud : clouds) {
            if (cloud.entityTags().contains(requiredTag) && isEntityInCloud(entity, cloud)) {
    ...
    public static AreaEffectCloud findCloudByUUID(LivingEntity entity, String uuid) {
        ...
        List<AreaEffectCloud> clouds = entity.level().getEntitiesOfClass(AreaEffectCloud.class, box,
                c -> c.entityTags().contains(TAG_STEAM_CLOUD) && c.entityTags().contains(targetTag));
        for (AreaEffectCloud cloud : clouds) {
            if (isEntityInCloud(entity, cloud)) {
                return cloud;
            }
