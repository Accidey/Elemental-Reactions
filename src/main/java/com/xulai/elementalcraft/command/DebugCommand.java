...
public class DebugCommand {
                        ...
                        .then(Commands.literal("debug")
                                .executes(context -> {
                                    ...
                                    if (!wasEnabled && nowEnabled) {
                                        player.sendSystemMessage(Component.translatable("command.elementalcraft.debug.enabled").withStyle(ChatFormatting.GREEN));
                                        player.sendSystemMessage(Component.translatable("command.elementalcraft.debug.global_notice").withStyle(ChatFormatting.BOLD, ChatFormatting.YELLOW));
                                    } else if (wasEnabled && !nowEnabled) {
                                        player.sendSystemMessage(Component.translatable("command.elementalcraft.debug.disabled").withStyle(ChatFormatting.RED));
                                    }
                                    return 1;
                                })
                        )
    ...
    public static long getRemainingCooldown(LivingEntity entity, String nbtKey) {
        ...
        long endTick = data.getLongOr(nbtKey, 0L);
    ...
    public static int getRemainingCooldownCountdown(LivingEntity entity, String nbtKey) {
        CompoundTag data = entity.getPersistentData();
        if (!data.contains(nbtKey)) return 0;
        return Math.max(0, data.getIntOr(nbtKey, 0));
    }
